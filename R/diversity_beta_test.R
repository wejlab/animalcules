...
diversity_beta_test <- function(MAE, 
  ...
  # organism x taxlev
  tax_table_init <- microbe %>% SummarizedExperiment::rowData() %>% as.data.frame()
  # sample x condition
  sam_table <- microbe %>% SummarizedExperiment::colData() %>% as.data.frame()
  # organism x sample
  counts_table_init <- microbe %>% SummarizedExperiment::assays() %>%
    as.data.frame() %>% dplyr::select(all_of(rownames(sam_table)))
  ...
  tax_table <- tax_table_init %>%
    dplyr::select(1:dplyr::all_of(tax_level)) %>%
  ...
  if (input_beta_method %in% c("bray", "jaccard")){
    ...
    dist.mat <- counts_table %>% t() %>%
      vegan::vegdist(method = input_beta_method) %>% as.matrix()
  } else stop("input_beta_method not recognized.")
  ...
  if (input_select_beta_stat_method == "PERMANOVA") {
    ...
    dist.list <- list(dist.within.a, dist.within.b, dist.between) %>%
      magrittr::set_names(c(unique(sam_table$condition)[1],
                            unique(sam_table$condition)[2],
                            "between"))
    if (input_select_beta_stat_method == "Wilcoxon rank sum test") {
      ...
      tmp <- list(dist.within.a, dist.within.b, dist.between) %>%
        kruskal.test()
      output <- c(tmp$method, tmp$p.value) %>% data.frame %>%
