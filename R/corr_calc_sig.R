...
#' @param ps A matrix of the p-values for each correlation
#' @param adjust p-value correction method. Select one from `p.adjust.methods`
#' @param no.sig Minimum number of significant correlations to be extracted. Default is set to 0.05
#' @param alpha Threshold for significance. Default is set to 0.05
...
calc_sig <- function(cors, ps, adjust, no.sig = 1, alpha = 0.05) {
  # Pre-allocating memory Empty correlation matrix
  sub_cors <- matrix(0, nrow = nrow(cors), ncol = ncol(cors))
  rownames(sub_cors) <- rownames(cors)
  colnames(sub_cors) <- colnames(cors)
  # Empty p-value matrix
  corrected_ps <- matrix(0, nrow = nrow(ps), ncol = ncol(ps))
  ...
  # 1) Extracting row + column indices from sig_rows + sig_ps 2) finding
  # them in cors 3) putting them into sub_cors
  ...
  colz <- which(MatrixGenerics::colSums2(sub_cors, na.rm = T) != 0)
  rowz <- which(MatrixGenerics::rowSums2(sub_cors, na.rm = T) != 0)
